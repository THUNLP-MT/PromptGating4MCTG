...
exp_name=train_generation_tense_bs1024_pastonly_yelp

CUDA_VISIBLE_DEVICES=$devices \
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$cuda_dir \
PYTHONPATH=$code_dir/ \
python $code_dir/thumt_gen/bin/trainer.py \
    --input $train_dir/past_label_train.spm.txt $train_dir/past_sent_train.spm.txt \
    --validation $train_dir/past_label_valid.spm.txt $train_dir/past_sent_valid.spm.txt \
    --reference $train_dir/past_sent_valid.spm.txt \
    --output train_gen4 \
...
mv log_gen5 train_gen4
if [ ! -d ../$exp_name ]; then
    mkdir ../$exp_name
fi
mv train_gen4 ../$exp_name
