...
exp_name=train_generation_topic_bs1024_mexionly_yelp_bal

CUDA_VISIBLE_DEVICES=$devices \
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$cuda_dir \
PYTHONPATH=$code_dir/ \
python $code_dir/thumt_gen/bin/trainer.py \
    --input $train_dir/mexi_label_train.spm.txt $train_dir/mexi_sent_train.spm.txt \
    --validation $train_dir/mexi_label_valid.spm.txt $train_dir/mexi_sent_valid.spm.txt \
    --reference $train_dir/mexi_sent_valid.spm.txt \
    --output train_gen \
...
mv log_gen train_gen
if [ ! -d ../$exp_name ]; then
    mkdir ../$exp_name
fi
mv train_gen ../$exp_name
