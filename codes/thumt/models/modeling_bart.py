...
from thumt.models.PrefixTuning import SegmentEmbedding
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        self.fr_encoder = nn.ModuleList([EncoderLayer(config) for _ in range(6)])
        ...
        for ide, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn = encoder_layer(x, attention_mask,
                                        ...
                                        prefixkv=prefixkv[:, ide, :] if prefixkv is not None else None)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x, self_attn_weights = self.self_attn(
            ...
            output_attentions=output_attentions,
            prefix_kv=None # (prefix_k, prefix_v)
        )
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        if not self.normalize_before:
