...
from thumt.models.PrefixTuning import PromptEncoder, GatingModule, PrefixTuning
...
class mBART(modules.Module):
    ...
    def __init__(self, params, name="mBART"):
        ...
        with utils.scope(name):
            ...
            if not params.only_prompt and not params.only_prefix:
                self.prompt_enc = PromptEncoder(params.hidden_size, params.prompt_encoder_filter_size, prompt_num=params.prompt_num, seg_num=params.source_num, pri_dim=params.prompt_encoder_pri_size) #TODO: format PET
    ...
    def encode(self, features, state, mode="train", mask_percentage=0):
        ...
        src_mask = torch.cat([torch.ones([src_mask.size()[0], self.params.prompt_num * sum(self.prompt_attached)], dtype=torch.long), src_mask], axis=1)
        ...
        encoder_output_all = self.mbart_encoder(input_ids={"src":features["source"] if self.src_attached[0] else None,
                                                           "hypo":[hyp for idp, hyp in enumerate(features["hypothesis"]) if self.src_attached[idp+1]], 
                                                ...
                                                fr_mask=features["hypothesis_mask"][-1])[0]
