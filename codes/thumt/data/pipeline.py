...
class MTPipeline(object):
    ...
    def get_train_dataset(filenames, params, cpu=False):
        ...
        src_dataset = src_dataset.tokenize(WhiteSpaceTokenizer(),
                                           None, [params.eos, params.src_lang_tok])
        hyp_dataset = [hyp.tokenize(WhiteSpaceTokenizer(), None, [params.eos, params.hyp_lang_tok[idp]]) for idp, hyp in enumerate(hyp_dataset)] if hyp_dataset is not None else None
        tgt_dataset = tgt_dataset.tokenize(WhiteSpaceTokenizer(),
                                           params.tgt_lang_tok, params.eos)
        lab_dataset = lab_dataset.tokenize(WhiteSpaceTokenizer(),
                                           None, [params.eos, params.tgt_lang_tok])
    ...
    def get_infer_dataset(filename, params, cpu=False):
        ...
        hyp_dataset = [TextLineDataset(sorted_data[idp]) for idp in range(1, len(sorted_data))] if len(sorted_data) > 1 else None
        src_dataset = src_dataset.tokenize(WhiteSpaceTokenizer(),
                                           None, [params.eos, params.src_lang_tok])
        hyp_dataset = [hyp.tokenize(WhiteSpaceTokenizer(), None, [params.eos, params.hyp_lang_tok[idp]]) for idp, hyp in enumerate(hyp_dataset)] if hyp_dataset is not None else None
        ...
        dataset = Dataset.zip((src_dataset, ) + tuple(hyp_dataset))
        ...
        def map_fn(inputs):
            src_seq = inputs[0]
            hyp_seq = inputs[1:] if len(inputs) > 1 else None
            src_seq = torch.tensor(src_seq)
            if hyp_seq is not None:
                hyp_seq = [torch.tensor(hyp) for hyp in hyp_seq]
            src_mask = src_seq != params.vocabulary["source"][params.pad]
