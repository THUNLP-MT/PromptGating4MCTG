...
exp_name=train_generation_tense_bs1024_futureonly_yelp

CUDA_VISIBLE_DEVICES=$devices \
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$cuda_dir \
PYTHONPATH=$code_dir/ \
python $code_dir/thumt_gen/bin/trainer.py \
    --input $train_dir/future_label_train.spm.txt $train_dir/future_sent_train.spm.txt \
    --validation $train_dir/future_label_valid.spm.txt $train_dir/future_sent_valid.spm.txt \
    --reference $train_dir/future_sent_valid.spm.txt \
    --output train_gen2 \
