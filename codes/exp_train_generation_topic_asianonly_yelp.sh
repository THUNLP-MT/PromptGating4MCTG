...
exp_name=train_generation_topic_bs1024_asianonly_yelp_bal

CUDA_VISIBLE_DEVICES=$devices \
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$cuda_dir \
PYTHONPATH=$code_dir/ \
python $code_dir/thumt_gen/bin/trainer.py \
    --input $train_dir/asian_label_train.spm.txt $train_dir/asian_sent_train.spm.txt \
    --validation $train_dir/asian_label_valid.spm.txt $train_dir/asian_sent_valid.spm.txt \
    --reference $train_dir/asian_sent_valid.spm.txt \
    --output train_gen1 \
...
mv log_gen2 train_gen1
if [ ! -d ../$exp_name ]; then
    mkdir ../$exp_name
fi
mv train_gen1 ../$exp_name
