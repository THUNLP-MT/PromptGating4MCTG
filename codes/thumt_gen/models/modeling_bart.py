...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False, attach=None, adding=None, gating=None, prefixkv=None):
        ...
        x, attn_weights = self.self_attn(
            query=x, key=x, key_padding_mask=encoder_padding_mask, output_attentions=output_attentions, prefix_kv=prefixkv
        )
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        self.embed_segments = SegmentEmbedding(4, embed_dim)
        
        self.fr_encoder = nn.ModuleList([nn.ModuleList([EncoderLayer(config) for _ in range(6)])
                                         for _ in range(3)])
    ...
    def forward(
        ...
        # if pre_encode_fr:
        #     fr_mask = invert_mask(fr_mask)
        #     inputs_embeds_list[-1] = inputs_embeds_list[-1].transpose(0, 1)
        #     for ide, encoder_layer in enumerate(self.fr_encoder):
        #         dropout_probability = random.uniform(0, 1)
        #         if self.training and (dropout_probability < self.layerdrop):  # skip the layer
        #             _ = None
        #         else:
        #             inputs_embeds_list[-1], _ = encoder_layer(inputs_embeds_list[-1], fr_mask)
        #     inputs_embeds_list[-1] = inputs_embeds_list[-1].transpose(0, 1)
        if pre_encode_fr:
            src_id = 0
            for ide, en in enumerate(input_ids['attach']):
                if not en or ide >= len(self.fr_encoder):
                    continue
                if ide != 2:
                    src_id += 1
                    continue
                fr_mask[ide] = invert_mask(fr_mask[ide])
                inputs_embeds_list[src_id] = inputs_embeds_list[src_id].transpose(0, 1)
                for idl, encoder_layer in enumerate(self.fr_encoder[ide]):
                    dropout_probability = random.uniform(0, 1)
                    if self.training and (dropout_probability < self.layerdrop):
                        _ = None
                    else:
                        inputs_embeds_list[src_id], _ = encoder_layer(inputs_embeds_list[src_id], fr_mask[ide])
                inputs_embeds_list[src_id] = inputs_embeds_list[src_id].transpose(0, 1)
                src_id += 1
        ...
        if len(inputs_embeds_list) == 0:
            ...
            inputs_embeds = torch.cat(inputs_embeds_list, axis=1)
        
        x = inputs_embeds + embed_seg
        input_ids['attach'] = [bool(x) for x in input_ids['attach']]
        if prompt_enc is not None:
            prompt = prompt_enc(input_ids['attach'])
        ...
        adding, gating = None, None
        if gating_enc is not None:
            gating = gating_enc(input_ids['attach'])
            if gating is not None:
                adding, gating = gating
        
        prefixkv = None
        if prefix_enc is not None:
            prefixkv = prefix_enc(input_ids['attach'])
        ...
        for ide, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn = encoder_layer(x, attention_mask,
                                        ...
                                        adding=adding[:, ide, :] if adding is not None else None, 
                                        gating=gating[:, ide, :] if gating is not None else None,
                                        prefixkv=(prefixkv[0][:, ide, :], prefixkv[1][:, ide, :]) if prefixkv is not None else None)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # prefix_k, prefix_v = self.self_prefix()
        ...
        x, self_attn_weights = self.self_attn(
            ...
            output_attentions=output_attentions,
            prefix_kv=prefix_kv[0] if prefix_kv is not None else None # (prefix_k, prefix_v)
        )
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        if not self.normalize_before:
        ...
        # reciever_addition, reciever_gate = self.receiver()
        # if reciever_addition is not None:
        #     reciever_addition = reciever_addition.unsqueeze(0).expand(x.size(1), -1, -1).transpose(0, 1)
        #     reciever_gate = reciever_gate.unsqueeze(0).expand(x.size(1), -1, -1).transpose(0, 1)
        #     reciever_addition = torch.cat([reciever_addition, torch.zeros([x.size(0)-reciever_addition.size(0), x.size(1), x.size(2)])], axis=0)
        #     reciever_gate = torch.cat([reciever_gate, torch.ones([x.size(0)-reciever_gate.size(0), x.size(1), x.size(2)])], axis=0)

        #     x = reciever_addition + (x*reciever_gate)

        # Cross attention
        residual = x
        assert self.encoder_attn.cache_key != self.self_attn.cache_key

        # prefix_k, prefix_v = self.receiver()
        ...
        x, _ = self.encoder_attn(
            ...
            prefix_kv=prefix_kv[1] if prefix_kv is not None else None #(prefix_k, prefix_v)
        )
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        if not self.normalize_before:
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        **unused,
        ...
        #     receiver = receiver()
        #     if receiver is not None:
        #         receiver = receiver.expand(x.size()[0], -1, -1)
        #         x = torch.cat([receiver, x], axis=1)
        ...
        prefixkv = None
        if prefix_dec is not None:
            prefixkv = prefix_dec(attach)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_self_attn, layer_past = decoder_layer(
                ...
                output_attentions=output_attentions,
                prefix_kv=((prefixkv[0][:, idx*2, :], prefixkv[1][:, idx*2, :]), (prefixkv[0][:, idx*2+1, :], prefixkv[1][:, idx*2+1, :])) if prefixkv is not None else None,
            )
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if prefix_kv is not None:
            prefix_k = prefix_kv[0].unsqueeze(0).expand(bsz, -1, -1).reshape(bsz*self.num_heads, -1, self.head_dim)
        src_len = k.size(1)
        # print(q.size(), k.size())

        attn_weights = torch.bmm(q, k.transpose(1, 2))
        if prefix_kv is not None:
            prefix_weights = torch.bmm(q, prefix_k.transpose(1, 2))
        ...
        if key_padding_mask is not None:  # don't attend to padding symbols
            ...
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        attn_weights = F.softmax(torch.cat([prefix_weights, attn_weights], axis=2) if prefix_kv is not None else attn_weights, dim=-1)
        ...
        if prefix_kv is not None:
            v = torch.cat([prefix_kv[1].unsqueeze(0).expand(bsz, -1, -1).reshape(bsz*self.num_heads, -1, self.head_dim), v], axis=1)
