...
import thumt_gen.optimizers as optimizers
import thumt_gen.utils as utils
import thumt_gen.utils.summary as summary

from thumt_gen.utils.moses import MosesTokenizer
...
def load_references(pattern, tools):
    if not pattern:
        return None
        ...
        with open(name, "rb") as fd:
            for line in fd:
                if 'tokenizer' in tools and 'sentence_piece_model' in tools:
                    ...
                    items = tools['sentence_piece_model'].decode(line.strip().split())
                    items = tools['tokenizer'].tokenize(items, return_str=True)
                    items = items.encode('utf-8').split()
                else:
...
def main(args):
    ...
    if params.spm_path:
        tools = {'tokenizer': MosesTokenizer(), 
                 'sentence_piece_model': spm.SentencePieceProcessor(model_file=params.spm_path)}
    else:
        tools = {}

    if params.validation:
        sorted_key = None
        eval_dataset = data.MTPipeline.get_train_dataset(
            params.validation, params)
        references = load_references(params.references, tools)
    else:
        sorted_key = None
        eval_dataset = None
        references = None
    ...
    if args.checkpoint is not None:
        ...
        step = state["step"] if "step" in state else params.initial_step
    ...
    def train_fn(inputs, step):
        features, labels = inputs
        loss = model(features, labels, 1.0-step/params.train_steps)
        return loss
            ...
            if counter % params.update_cycle == 0:
                if step >= params.train_steps:
                    utils.evaluate(model, sorted_key, eval_dataset,
                                   params.output, references, params, tools)
