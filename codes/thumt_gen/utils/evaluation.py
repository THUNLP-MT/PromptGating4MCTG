...
from thumt_gen.utils.inference import beam_search
from thumt_gen.utils.bleu import bleu
from thumt_gen.utils.bpe import BPE
from thumt_gen.utils.misc import get_global_step
from thumt_gen.utils.summary import scalar
...
def _spm_decode_and_tokenize(seq, tokenizer, spm):
    seq = seq.decode('utf-8')
    seq = spm.decode(seq.split())
    seq = tokenizer.tokenize(seq, return_str=True)
    seq = seq.encode('utf-8')
    return seq

def _evaluate_model(model, sorted_key, dataset, references, params, tools):
    with torch.no_grad():
        if params.validation_mode == "loss":
        # Create model
            ...
            while True:
                ...
                # Decode
                # _features, _label = {"source":features["source"].clone(),
                #                      "source_mask":features["source_mask"].clone(),
                #                      "hypothesis":features["hypothesis"].clone() if features["hypothesis"] is not None else None,
                #                      "hypothesis_mask":features["hypothesis_mask"].clone() if features["hypothesis_mask"] is not None else None,
                #                      "target":features["target"].clone(),
                #                      "target_mask":features["target_mask"].clone()}, label.clone()
                _loss, _cnt = model(features, label, mode="valid")
        ...
        while True:
            ...
            for i in range(params.decode_batch_size):
                for j in range(dist.get_world_size()):
                    ...
                    if 'tokenizer' in tools and 'sentence_piece_model' in tools:
                        tokenizer = tools['tokenizer']
                        spm = tools['sentence_piece_model']
                        seq = _spm_decode_and_tokenize(seq, tokenizer, spm)
                    else:
...
def evaluate(model, sorted_key, dataset, base_dir, references, params, tools):
    if not references:
        return
    ...
    score = _evaluate_model(model, sorted_key, dataset, references, params, tools)
